...
export const renderBlock = (block: any) => {
  switch (block.type) {
    ...
    case 'bookmark':
      ...
      return (
        <a href={bookmark.url} target="_blank" rel="noopener noreferrer" className="block my-4 p-4 border rounded-md hover:bg-gray-50">
    ...
    case 'column_list':
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-4">
          {block.children?.map((columnBlock: any) => (
            <div key={columnBlock.id} className="flex flex-col">
              {columnBlock.children?.map((childBlock: any) => (
